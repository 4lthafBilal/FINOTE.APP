...
<head>
    ...
    <title>FINOTE - Laravel Version</title>
    ...
    <div class="flex min-h-screen">
        <aside class="w-64 bg-white border-r flex flex-col p-6 fixed h-full shadow-sm">
            ...
            <div class="border-t pt-4">
                {{-- Logout menggunakan standar Laravel --}}
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="text-red-500 font-bold p-3 hover:bg-red-50 rounded-xl transition-all text-sm block w-full text-left">
                        Keluar
                    </button>
                </form>
            </div>
    ...
    <script>
        function app() {
            return {
                ...
                {{-- Memasukkan data transaksi ke format JSON AlpineJS --}}
                trxs: {!! json_encode($trxs) !!}, 
                nt: {t:'', a:'', tp:'in'},
                ...
                {{-- Fungsi Save Laravel akan diarahkan ke Route Laravel --}}
                async saveP() { 
                    const fd = new FormData(); 
                    fd.append('_token', '{{ csrf_token() }}'); {{-- Wajib ada CSRF di Laravel --}}
                    fd.append('username',this.user.n); fd.append('job',this.user.j); 
                    fd.append('photo',this.user.ph||''); fd.append('email',this.user.e);
                    const r = await fetch("{{ route('profile.update') }}",{method:'POST',body:fd});
                    if(r.ok) { alert('Profil Diperbarui!'); location.reload(); } else { alert('Gagal Simpan Profil'); }
                },
                async saveT() {
                    ...
                    await fetch("{{ route('transaction.store') }}",{method:'POST',body:fd}); 
                    location.reload();
                }
            }
        }
